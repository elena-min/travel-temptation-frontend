...
function getExcursionsByTravelAgency(travelAgencyID){
    return axios.get(`http://localhost:8080/excursions/travel-agency/${travelAgencyID}`)
        .then(response => response.data);
}
...
function searchExcursionsByNameAndTravelAgencyAndPrice(searchTerm, minPrice, maxPrice) {
    let url = `http://localhost:8080/excursions/search-name-and-price`;
...
function getBookingsByExcursion(excursionId){
    return axios.get(`http://localhost:8080/excursions/${excursionId}/bookings`)
        .then(response => response.data);   
}
